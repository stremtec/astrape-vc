"""Phase 1c: Pre-cached features, fast training."""
...
cv=Conv()
opt=torch.optim.AdamW(list(splitter.parameters())+list(cv.parameters()), lr=1e-3)
...
for step in range(80):
    ...
    for s,t,u in pairs:
        c_s,s_s,_=feat[(s,u)]; c_t,s_t,_=feat[(t,u)]; z_t=feat[(t,u)][2]
        ...
        zvc=cv(c_s[:,:,:T],s_t.squeeze()); Tq=min(zvc.shape[2],z_t.shape[2])
        loss=F.mse_loss(zvc[:,:,:Tq],z_t[:,:,:Tq])+0.3*loss_c+0.5*loss_s
        opt.zero_grad(); loss.backward()
        torch.nn.utils.clip_grad_norm_(list(splitter.parameters())+list(cv.parameters()),1.0); opt.step()
...
# Test
subprocess.run(['ffmpeg','-y','-i','/Users/asill/Downloads/origin.mp3','-ar','24000','-ac','1','-sample_fmt','s16','/tmp/tp3.wav'],capture_output=True)
def convert(sa,ta):
    with torch.no_grad():
...
tgt_c=load_any('/tmp/tp3.wav',dur=None)
...
for nm,ta in[('parallel',tgt_p),('cross',tgt_c)]:
    ...
    sf.write(f'{out}/phase1c_{nm}.wav',va[0,0,:Tc].numpy(),SR)
print('✅')
