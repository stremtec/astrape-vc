...
@torch.no_grad()
def collect_representations(encoder, loader, device, limit, time_shift=0):
    """Collect paired (teacher_content, student_content) over the dataset.
    
    time_shift: if the encoder was trained with --time-shift N, the student
    output at frame t aligns with the teacher at frame t-N. We realign by
    comparing student[ts:] with teacher[:-ts] (dropping the first ts student
    frames and the last ts teacher frames).
    """
    ...
    for batch in loader:
        ...
        for i in range(student.shape[0]):
            ...
            s = student[i, :T][valid]
            t = teacher[i, :T][valid]
            # time-shift realignment: student[t] ↔ teacher[t-ts]
            # so compare student[ts:] with teacher[:-ts]
            if ts > 0 and s.shape[0] > ts:
                s = s[ts:]
                t = t[:-ts]
            L = min(s.shape[0], t.shape[0])
            if L > 0:
                student_all.append(s[:L].cpu())
                teacher_all.append(t[:L].cpu())
...
def main():
    ...
    ap.add_argument("--wavlm-dir", default="wavlm_L4_200hz",
                    help="WavLM cache subdir under data-dir")
    ap.add_argument("--time-shift", type=int, default=None,
                    help="Time shift used in encoder training (auto-detected from checkpoint)")
    args = ap.parse_args()
    ...
    # Auto-detect time_shift from checkpoint if not explicitly overridden
    ck_meta = torch.load(str(args.encoder_ckpt), map_location="cpu", weights_only=False)
    saved_args = ck_meta.get("args", {})
    if args.time_shift is None:
        args.time_shift = saved_args.get("time_shift", 0)
    print(f"  time_shift = {args.time_shift}", flush=True)
    ...
    # If encoder uses WavLM frontend, wrap dataset to replace mel with cached WavLM
    if config.use_wavlm_frontend:
        from .train_encoder import WavLMFrontendDataset
        wavlm_dir = args.wavlm_dir
        ds = WavLMFrontendDataset(ds, args.data_dir, wavlm_dir)
        frames_per_token = config.wavlm_rate // 25
        print(f"  WavLM frontend: using cached {wavlm_dir} (512d) instead of mel",
              flush=True)
    else:
        frames_per_token = 2
