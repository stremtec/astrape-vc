"""Train MCS-Decoder v3 by end-to-end output distillation from the frozen MioCodec teacher.

Loss (output only — the architecture differs from the teacher, so we match its OUTPUT, not
its internals): MR-STFT (main) + Mel-L1 (aux) + small complex-STFT (phase nudge, 0.01).
No GAN, no feature distill. Spectral losses on CPU (MPS torch.stft backward is non-finite).

  .venv/bin/python -m astrape.train_mcs_decoder --device mps --epochs 80 \
      --content-dir /Users/asill/btrv5_content --num-workers 4 \
      --out-dir /Volumes/UNTITLED/btrv5_checkpoints/mcs_v3
"""
...
from .mcs_decoder import MCSDecoderV3, MCSDecoderConfig
...
def main():
    ...
    dec = MCSDecoderV3(MCSDecoderConfig()).to(device)
    ...
    print(f"MCSDecoderV3: {sum(p.numel() for p in dec.parameters())/1e6:.2f}M  "
    ...
    for ep in range(start, args.epochs):
        ...
        for content, audio, speaker, _i in loader:
            ...
            if steps % 100 == 0:
                ...
                print(f"E{ep:02d} {steps:04d}/{args.steps_per_epoch} stft={acc['stft'].item()/d:.3f} "
                      f"mel={acc['mel'].item()/d:.3f} cstft={acc['cstft'].item()/d:.3f} skip={skipped} "
        ...
        ckpt = {"state_dict": dec.state_dict(), "opt": opt.state_dict(), "sch": sch.state_dict(),
                "decoder_config": MCSDecoderConfig().__dict__, "epoch": ep}
        ...
        (args.out_dir / "summary.json").write_text(json.dumps(
            {"epoch": ep, "skipped": skipped, **{k: v.item() / max(steps - skipped, 1) for k, v in acc.items()}}, indent=2) + "\n")
