#!/usr/bin/env python3
"""V3-4k: Same architecture, 4360 samples. Content embedding distillation only."""
...
BATCH=2; EPOCHS=40; device='cpu'
MAX_MEL_FRAMES=80  # ~1.6s at 50Hz — limit seq length for speed
...
meta=np.load(f"{OUT_DIR}/meta.npz"); n=meta['n_samples'].item(); spk_names=meta['spk_names'][:n]
...
def load(idx):
    md=np.load(f"{MEL_DIR}/m_{idx:05d}.npz"); od=np.load(f"{OUT_DIR}/s_{idx:05d}.npz")
    return (torch.from_numpy(md['logmel']).float(),torch.from_numpy(od['ce_768']).float(),
            torch.from_numpy(od['pre_fsq_768']).float())
...
print("Params:",sum(p.numel() for p in model.parameters()),"| V3-4k training...")

for epoch in range(EPOCHS):
    tr_l=0; nb=0; perm=np.random.permutation(len(tr))
    for i in range(0,len(perm),BATCH):
        bi=perm[i:i+BATCH]; mels=[]; ces=[]; pfs=[]
        for j in bi: 
            mel,ce,pf=load(tr[j])
            if mel.shape[1]>MAX_MEL_FRAMES:
                start=np.random.randint(0,mel.shape[1]-MAX_MEL_FRAMES)
                mel=mel[:,start:start+MAX_MEL_FRAMES]
                t25_start=start//2; t25_len=MAX_MEL_FRAMES//2
                ce=ce[t25_start:t25_start+t25_len]
                pf=pf[t25_start:t25_start+t25_len]
            mels.append(mel); ces.append(ce); pfs.append(pf)
...
os.makedirs("checkpoints",exist_ok=True); torch.save(model.state_dict(),"checkpoints/causal_student_v3_4k.pt")
print("Saved: causal_student_v3_4k.pt"); print("Done!")
